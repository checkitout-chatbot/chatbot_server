...
#2000년도부터 2022년도까지의 영화 정보 추출
#이미지 파일은 따로 jpg 형식으로 저장했으나, 나중에 이미지 링크만 따로 크롤링해도 괜찮을 듯
#csv 파일은 ,로 데이터를 구분하나, 기존 데이터에 ','가 포함되어 있으므로 '\t'로 구분함 
# => 엑셀 파일로 열어서 보는 데는 문제가 없지만 sql로 데이터베이스에 저장할 때는 문제가 생김


savePath = r'C:\Users\YEBIN\Desktop\한이음\영화csv-1'
...
def getMovieCodeByYear(year):
    ...
    if response.status_code == 200:
        ...
        # html에서 div 태그 중 class이름이 pagenavigation 태그
        ...
        for i in range(1, int(lastPage)+1):
            ...
            if response2.status_code == 200:
                ...
                # html에서 ul 태그 중 class이름이 directory_list 태그
...
def getMovieInfo(year_codes):
    ...
    f = open(os.path.join(savePath, f'{year}.csv'), 'w', encoding='utf-8-sig')
